...
// ── 앱 시작 시 백그라운드 자동 동기화 ──────────────────────────────────────────
// UI 모달 없이 조용히 전체 pull → 실시간 리스너로 커버 못한 이전 데이터 보완
async function _autoSyncOnStartup() {
  const workspaces = _getWorkspaces();
  if (!workspaces.length) return;
  if (!_fbReady) return;

  console.info('[납품] 앱 시작 자동 동기화 시작');
  try {
    const napumDb = _getNapumApp().database();
    const nameToId = {};
    S.clients.forEach(c => { nameToId[c.name] = c.id; });

    // _napumId → CRM transaction id 맵 (실제 존재하는 것만)
    const napumIdToCrmId = {};
    S.transactions.forEach(t => { if (t._napumId) napumIdToCrmId[t._napumId] = t.id; });

    // synced Set 재계산: _napumId가 있고 실제 transaction도 존재하는 것만 유효
    const validNapumIds = new Set(
      S.transactions.filter(t => t._napumId).map(t => t._napumId)
    );
    // synced LS는 참고만 하고 실제 존재 여부로 판단
    const synced = new Set(lsGet('crm_napum_synced', []));

    let totalNew = 0, totalUpd = 0, totalRestore = 0;
    ...
    for (const ws of workspaces) {
      try {
        ...
        const allowedClients = scSnap.exists() ? (scSnap.val() || []) : [];
        const ordersObj      = ordSnap.val() || {};
        const napumClients   = csSnap.val() || {};

        // 거래처 자동 추가
        Object.values(napumClients).forEach(nc => {
          if (!nc.name || nameToId[nc.name]) return;
          const newId = nextId(S.clients);
          const newC  = { id: newId, name: nc.name, phone: nc.phone || '', memo: '', group: '' };
          S.clients         = [...S.clients, newC];
          nameToId[nc.name] = newId;
          _saveOneClient(newC);
        });

        // orders → CRM transactions 비교 반영
        for (const [orderId, o] of Object.entries(ordersObj)) {
          if (!o || !o.clientName) continue;
          // 외부 ws 필터
          if (allowedClients.length > 0 && !allowedClients.includes(o.clientName)) continue;

          const napumKey = `${ws.id}:${orderId}`;
          const status   = o.isPaid ? TX_STATUS.PAID : TX_STATUS.UNPAID;
          const clientId = nameToId[o.clientName];
          if (!clientId) continue;

          const crmId    = napumIdToCrmId[napumKey];
          const existing = crmId ? S.transactions.find(t => t.id === crmId) : null;

          if (existing) {
            // ── 케이스 1: CRM에 있음 → 상태만 비교 업데이트
            if (existing.status !== status) {
              const updated = { ...existing, status };
              S.transactions = S.transactions.map(t => t.id === crmId ? updated : t);
              _saveOneTx(updated);
              totalUpd++;
            }
          } else {
            // ── 케이스 2: CRM에 없음 (신규 or synced에 있었지만 실제 누락)
            const isRestore = validNapumIds.has(napumKey) || synced.has(napumKey);
            const amount    = (o.items || []).reduce((s, it) => s + (it.qty || 0) * (it.price || 0), 0);
            const newTx     = {
              id: nextId(S.transactions),
              date: o.date || o.createdAt?.split('T')[0] || new Date().toISOString().split('T')[0],
              clientId, type: '매출', amount, tax: 0,
              memo: o.memo || '', status, _napumId: napumKey,
            };
            S.transactions        = [...S.transactions, newTx];
            napumIdToCrmId[napumKey] = newTx.id;
            validNapumIds.add(napumKey);
            _saveOneTx(newTx);
            synced.add(napumKey);
            if (isRestore) totalRestore++; else totalNew++;
          }
        }
      } catch(e) {
        console.warn('[납품] 자동동기화 워크스페이스 오류:', ws.id, e.message);
      }
    }

    lsSet('crm_napum_synced', [...synced]);

    const changed = totalNew + totalUpd + totalRestore;
    if (changed > 0) {
      render();
      const parts = [];
      if (totalNew     > 0) parts.push(`신규 ${totalNew}건`);
      if (totalRestore > 0) parts.push(`복구 ${totalRestore}건`);
      if (totalUpd     > 0) parts.push(`업데이트 ${totalUpd}건`);
      showToast(`📦 납품 동기화 완료 (${parts.join(', ')})`, 3500);
    }
    console.info('[납품] 자동 동기화 완료 — 신규:', totalNew, '복구:', totalRestore, '업데이트:', totalUpd);
  } catch(e) {
    console.warn('[납품] 자동 동기화 실패:', e.message);
  }
}
